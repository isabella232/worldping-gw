...
func (t *TsdbUsage) process() {
	...
	for {
		...
		case ts := <-ticker.C:
			...
			default:
				glog.Warning("Usage: output buffer full.")
			}
		case <-t.shutdown:
			select {
		...
		for _, data := range output.buf {
			for {
				...
				if err != nil {
					t.conn.Close()
					t.conn = nil
					glog.Errorf("error while writting to connection. %v", err)
					if deadline.Sub(time.Now()) < time.Duration(0) {
						glog.Errorf("Usage: failed to write data before deadline.")
						continue LOOP
					}
					connected := t.reconnect(deadline)
					if !connected {
...
func (t *TsdbUsage) reconnect(deadline time.Time) bool {
	...
	for deadline.Sub(time.Now()) > time.Duration(0) {
		...
		t.conn = nil
		glog.Errorf("failed to connect to tsdb-usage server. %v", err)
